function [v, c, lines_separating_points, points_separated] = find_lines_separating_points_170819a(XY) 
...
% identify lines separating points
lines_separating_points = [];
points_separated = [];

for i=1:size(c,1)
    for j=1:size(c,1)
        if j < i
            int = intersect(c{i}, c{j});
            if length(int) == 2
                lines_separating_points = [lines_separating_points; int];
                points_separated = [points_separated; i j];
            end
        end
    end
end